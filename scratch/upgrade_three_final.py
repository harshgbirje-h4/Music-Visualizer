...
function setupThreeScene() {
  ...
  // Tone mapping for rich color without washing out
  ...
  // Bloom configured for neon cores: radius 0.6, threshold 0.2
  ...
  // Streaks via PointsMaterial (could use lines, but Points with additive is fast)
...
function updateThreeVortexAudio() {
  ...
  let colorVio = new THREE.Color(0x8a2bff);
  ...
  if (state.autoCycle && state.theme !== 'bw') {
    ...
    colorVio.setHSL(((state.colorHue + 300) % 360)/360, 1.0, 0.5);
  } else if (!state.autoCycle && state.theme !== 'vortex') {
    // Tasteful fallback blending while keeping cyberpunk
  ...
  // Moderate intensity multipliers to prevent white blowout
  // Base 1.0, add max 0.3 from bass and 0.5 from beat
  const intensity = 1.0 + (bassPulse * 0.3) + (beatDecay * 0.5);
  
  data.vortexGates.forEach(g => {
    g.materials[0].color.copy(colorMag).multiplyScalar(intensity);
    g.materials[1].color.copy(colorCyan).multiplyScalar(intensity * 1.1); // cyan slightly brighter
  ...
  if (data.vortexParticles) {
    data.vortexParticles.material.color.copy(colorCyan).multiplyScalar(intensity * 1.2);
  }

  // Bloom tuning for premium glow without overexposure
  data.bloomPass.strength = 1.1 + (bassPulse * 0.3) + (beatDecay * 0.3);
...
print("Successfully applied 10/10 final Three.js Vortex upgrade!")
