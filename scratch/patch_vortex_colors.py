...
function drawCorridorSurfaces(c, z0, z1, fog, primaryColor, secondaryColor, accentColor, mid, beatGlow, camX, camY, cx, cy, focalLength, nearClip) {
  ...
  for (let col of floorCols) {
    ...
    drawNeonLine(c, pl0, pl1, primaryColor, pbL0.scale * 0.002, fog * 0.4 * (0.5 + beatGlow * 0.6));
  }
  
  if (Math.round(z0 * 2) % 2 === 0) {
     drawNeonLine(c, pbL0, pbR0, primaryColor, pbL0.scale * 0.0025, fog * 0.6 * (0.8 + beatGlow * 0.4));
  }
  ...
  drawNeonLine(c, wmL0, wmL1, secondaryColor, ptL0.scale * 0.002, fog * 0.5 * (0.4 + mid * 0.6 + beatGlow * 0.3));
  drawNeonLine(c, wmR0, wmR1, secondaryColor, ptL0.scale * 0.002, fog * 0.5 * (0.4 + mid * 0.6 + beatGlow * 0.3));
  
  if (Math.round(z0 * 3) % 3 === 0) {
    drawNeonLine(c, ptL0, pbL0, secondaryColor, ptL0.scale * 0.0015, fog * 0.4 * (0.8 + beatGlow * 0.4));
    drawNeonLine(c, ptR0, pbR0, secondaryColor, ptL0.scale * 0.0015, fog * 0.4 * (0.8 + beatGlow * 0.4));
  }
  ...
  for (let col of ceilCols) {
    ...
    drawNeonLine(c, cl0, cl1, accentColor, ptL0.scale * 0.003, fog * 0.7 * (0.8 + beatGlow * 0.4));
  }

  c.restore();
}

function drawFloorReflections(c, z0, z1, depthAlpha, gateColor, beatGlow, camX, camY, cx, cy, focalLength, nearClip) {
  const alpha = (0.15 + beatGlow * 0.25) * depthAlpha;
...
function drawVortex(c, width, height) {
  ...
  const beatGlow = clamp(state.vortexBeatGlow, 0, 1);
  ...
  const isCustomOrBW = state.theme === 'bw' || state.theme === 'custom' || state.theme === 'rock' || state.theme === 'chill' || state.theme === 'phonk' || state.theme === 'memory' || state.theme === 'classic' || state.theme === 'study' || state.theme === 'default';

  let primaryColor, secondaryColor, accentColor, fogAmbientColor, gateAccentColor;

  if (state.autoCycle) {
    primaryColor = paletteColor(0, 1);
    secondaryColor = paletteColor(0.33, 1);
    ...
    accentColor = '#8a2bff';
    gateAccentColor = '#ff008c';
    fogAmbientColor = '#4ddcff';
  } else {
    const tConfig = themeConfig();
    primaryColor = tConfig.palette[0] || '#ffffff';
    secondaryColor = tConfig.palette[1] || '#aaaaaa';
  ...
  fogGrad.addColorStop(0, cyberAlpha(fogAmbientColor, 0.15 + (state.bassMode ? bass * 0.05 : 0) + beatGlow * 0.1));
  fogGrad.addColorStop(0.3, cyberAlpha(primaryColor, 0.08 + (state.bassMode ? bass * 0.03 : 0)));
  ...
  for (let i = 0; i < quality.gates; i++) {
    ...
    const gateColorOuter = primaryColor;
    const gateColorInner = gate.colorSide === 0 ? secondaryColor : gateAccentColor;

    const pulse = state.bassMode ? (1.0 + bass * 0.07 + gate.pulse * 0.08 + beatGlow * 0.02) : (1.0 + gate.pulse * 0.04);

    projectGate(gate, pulse, camX, camY, cx, cy, focalLength, nearClip);

    drawCorridorSurfaces(c, z0, z1, fog, primaryColor, secondaryColor, accentColor, mid, beatGlow, camX, camY, cx, cy, focalLength, nearClip);

    drawFloorReflections(c, z0, z1, fog, gateColorOuter, beatGlow, camX, camY, cx, cy, focalLength, nearClip);

    const size = Math.max(1, _gateProjOuter[0].scale * 0.0035);
    drawGateStructure(c, fog);
    
    // Gate stroke, brighter on beatGlow
    const glowMult = 1.0 + beatGlow * 0.5;
    drawNeonPath(c, _gateProjOuter, gateColorOuter, size * 1.5, fog * glowMult, true);
    drawNeonPath(c, _gateProjInner, gateColorInner, size * 0.9, fog * glowMult, true);
    ...
    for (let pIdx = 0; pIdx < quality.particles; pIdx++) {
      ...
      if (isInside) {
        ...
        if (head && tail) {
          ...
          const pAlpha = pFog * pFog * (0.3 + high * 0.7 + beatGlow * 0.4);
          const pColor = particle.colorSide === 0 ? secondaryColor : primaryColor;
  ...
  const nebulaRadius = minDim * clamp(0.045 + (state.bassMode ? bass * 0.03 : 0) + beatGlow * 0.02, 0.035, 0.095);
  const coreGlow = c.createRadialGradient(cx, cy, 0, cx, cy, nebulaRadius);
  coreGlow.addColorStop(0, `rgba(235,250,255,${0.8 + beatGlow * 0.2})`);
  coreGlow.addColorStop(0.3, cyberAlpha(fogAmbientColor, 0.4 + (state.bassMode ? bass * 0.15 : 0) + beatGlow * 0.2));
  coreGlow.addColorStop(0.75, cyberAlpha(primaryColor, 0.15 + beatGlow * 0.1));
