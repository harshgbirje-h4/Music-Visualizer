...
function project3D(p, camera, fov, cx, cy) {
  ...
  // Base scale multiplier to make the tunnel visually perfectly sized.
...
function renderTunnelLayer(c, pat, alphaMult, ringData, now, bass, mids, width, SIDES) {
  ...
  if (pat === 0) {
    ...
    for (let i = 0; i < ringData.length; i++) {
      ...
      // Line width grows from ~2 at far plane (z=45) to ~40 at near plane (z=1)
      c.lineWidth = Math.max(2, 40 / r.z);
    ...
    for (let i = 0; i < ringData.length - 1; i++) {
      ...
      c.lineWidth = Math.max(1, 10 / r1.z);
      c.stroke();
    }
  }
  else if (pat === 2) {
    ...
    for (let i = 0; i < ringData.length - 1; i++) {
      ...
      c.shadowBlur = 20 / r1.z;
      c.shadowColor = `rgba(255, 255, 0, ${alpha})`;
      c.strokeStyle = `rgba(255, 255, 255, ${alpha})`;
      c.lineWidth = Math.max(1, 15 / r1.z);
      c.stroke();
      c.shadowBlur = 0;
...
function getPatternConfig(pat, width) {
  let SIDES = 8, RADIUS = 1.0;
  // Normalized world coordinates (1.0 is the base radius)
  if (pat === 0) { SIDES = 64; RADIUS = 1.0; }
  else if (pat === 1) { SIDES = 4; RADIUS = 1.2; }
  else if (pat === 2) { SIDES = 4; RADIUS = 1.1; }
  else if (pat === 3) { SIDES = 8; RADIUS = 1.1; }
  else if (pat === 4) { SIDES = 8; RADIUS = 1.0; }
  return { SIDES, RADIUS };
}
...
function renderInfinityTunnel(c, width, height) {
  ...
  // Smooth camera sway (normalized world units, tunnel radius is ~1.0)
  const camera = {
    x: Math.sin(now * 0.0006) * 0.25,
    y: Math.cos(now * 0.00045) * 0.25
  };
  ...
  if (vs.progress < 1) {
    // Transitioning: Crossfade current fading out, next fading in
    ...
    // Stable: Render current pattern fully visible
    const currentData = processPattern(vs.currentPat);
    renderTunnelLayer(c, vs.currentPat, 1.0, currentData.ringData, now, bass, mids, width, currentData.SIDES);
  }
