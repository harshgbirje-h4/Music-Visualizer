...
function updateThreeVortexColors() {
  ...
  state.threeVortexMaterials.floorLights.forEach(item => {
    ...
    // Dimmer floor lights to prevent blowout
...
function setupThreeScene() {
  ...
  // Center of the scene vertically is y = 13.
  ...
  for (let i = 0; i < data.segmentsCount; i++) {
    ...
    // Thinner, cleaner floor strips to reduce blowout
    const fMagL = new THREE.Mesh(boxGeo, fMagLMat);
    fMagL.scale.set(0.6, 0.1, data.spacing * 0.8);
    ...
    fMagR.scale.set(0.6, 0.1, data.spacing * 0.8);
    ...
    // Subtle Reflection Accents (much dimmer, cleaner)
    ...
    fReflGate.scale.set(36, 0.05, 1.5);
    fReflGate.position.set(0, 0.02, zPos - 2);
    scene.add(fReflGate);
    ...
    wCyanL.position.set(-39.5, 6, zPos - data.spacing / 2);
    scene.add(wCyanL);
    ...
    wCyanR.position.set(39.5, 6, zPos - data.spacing / 2);
    scene.add(wCyanR);
    data.vortexWallLights.push({ mesh: wCyanR, mat: wCyanRMat });

    const wMagVertL = new THREE.Mesh(boxGeo, wMagVertLMat);
    wMagVertL.scale.set(1.5, 20, 1.5);
    wMagVertL.position.set(-39.2, 13, zPos - data.spacing * 0.25);
    scene.add(wMagVertL);
    data.vortexWallLights.push({ mesh: wMagVertL, mat: wMagVertLMat });

    const wMagVertR = new THREE.Mesh(boxGeo, wMagVertRMat);
    wMagVertR.scale.set(1.5, 20, 1.5);
    wMagVertR.position.set(39.2, 13, zPos - data.spacing * 0.25);
    scene.add(wMagVertR);
    ...
    cCyanL.scale.set(0.8, 0.5, data.spacing);
    cCyanL.position.set(-14, 25.5, zPos - data.spacing / 2);
    scene.add(cCyanL);
    data.vortexCeilingLights.push({ mesh: cCyanL, mat: cCyanLMat });

    const cCyanR = new THREE.Mesh(boxGeo, cCyanRMat);
    cCyanR.scale.set(0.8, 0.5, data.spacing);
    cCyanR.position.set(14, 25.5, zPos - data.spacing / 2);
    scene.add(cCyanR);
    data.vortexCeilingLights.push({ mesh: cCyanR, mat: cCyanRMat });

    const cMagFrame = new THREE.Mesh(boxGeo, cMagFrameMat);
    cMagFrame.scale.set(40, 1.0, 2);
    cMagFrame.position.set(0, 25.5, zPos - data.spacing * 0.5);
    scene.add(cMagFrame);
    data.vortexCeilingLights.push({ mesh: cMagFrame, mat: cMagFrameMat });

    // ================= GATES (Perfectly Centered & Symmetrical) =================
    ...
    // Y center is 13. Top is 24, Bottom is 2. (Distance from center = 11)
    // X center is 0. Left is -20, Right is 20.
    const gTop = createBar(26, 3, 4, 0, 24, 0, matMain);
    const gBot = createBar(26, 3, 4, 0, 2, 0, matMain);
    
    // Side angle calculation to connect perfectly
    const gTL = createBar(4, 22, 4, -18.5, 17, -0.5, matMain);
    const gTR = createBar(4, 22, 4, 18.5, 17, 0.5, matMain);
    const gBL = createBar(4, 22, 4, -18.5, 9, 0.5, matMain);
    const gBR = createBar(4, 22, 4, 18.5, 9, -0.5, matMain);

    // Inner Cyan Lines
    const cTop = createBar(23, 1, 4.5, 0, 22.5, 0, matInner);
    const cBot = createBar(23, 1, 4.5, 0, 3.5, 0, matInner);
    const cTL = createBar(1.5, 19, 4.5, -16.5, 16.5, -0.5, matInner);
    const cTR = createBar(1.5, 19, 4.5, 16.5, 16.5, 0.5, matInner);
    const cBL = createBar(1.5, 19, 4.5, -16.5, 9.5, 0.5, matInner);
    const cBR = createBar(1.5, 19, 4.5, 16.5, 9.5, -0.5, matInner);
  ...
  for(let i=0; i<pCount; i++) {
    ...
    pPos[i*3+1] = Math.random() * 26; // Y from 0 to 26 (fill whole volume)
...
function updateThreeVortexAudio() {
  ...
  // Clean Audio intensity logic
  const baseEmissive = 1.0;
  // Floor gets less emissive boost so it doesn't blow out
  const floorBase = 0.5; 
...
print("Successfully injected symmetric composition fix!")
